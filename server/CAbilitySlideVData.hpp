...
public:
	...
	float m_flLandedFlatGroundFrictionGraceTime; // 0x1584	
	...
	float m_flSlideActivationProbeForwardOffset; // 0x15fc	
	// MPropertyDescription "Maximum distance between probe samples"
	float m_flMaxDistanceBetweenProbeSamples; // 0x1600	
	...
	float m_flSampleVelDiffStdDevScaleCutoff; // 0x160c	
	// MPropertyDescription "If facing further than this angle away from the direction of the slope, dont initiate a slide on it."
	float m_flSlopeFacingAngleToActivate; // 0x1610	
	// MPropertyDescription "Maximum air-drag to apply when jumping out of a slide."
	float m_flAirDragAfterJump; // 0x1614	
	// MPropertyDescription "How long to apply air-drag after jumping out of a slide."
	float m_flAirDragAfterJumpTime; // 0x1618	
	// MPropertyDescription "Air-drag applied when jumping out of a slide linearly ramps from 100% to 0% from 0' to this angle."
	float m_flAirDragMaxAngle; // 0x161c	
	// MPropertyDescription "How long you have to be walking around, not sliding until we clear the air-drag flag on jumps."
	float m_flAirDragResetTime; // 0x1620	
	// MPropertyDescription "How long after you stop sliding do we consider it to still be a slide-jump."
	float m_flLateSlideJumpWindow; // 0x1624	
	// MPropertyDescription "Speed remap to slide effect intensity"
	CRemapFloat m_SlideEffectRemap; // 0x1628	
	// MPropertyDescription "The curve defining the getup anim's speed penalty duration(x) and severity(y)."
	CPiecewiseCurve m_GetupSpeedCurve; // 0x1638	
	// MPropertyDescription "How long after you stop sliding do we consider it to still be a slide-jump."
	float m_flGetupBusyDuration; // 0x1678	
private:
	[[maybe_unused]] uint8_t __pad167c[0x4]; // 0x167c
public:
	// MPropertyStartGroup "Camera"
	CitadelCameraOperationsSequence_t m_cameraSequenceStartSliding; // 0x1680	
	CitadelCameraOperationsSequence_t m_cameraSequenceEndSliding; // 0x1708	
	// MPropertyStartGroup "Visuals"
	CResourceNameTyped< CWeakHandle< InfoForResourceTypeIParticleSystemDefinition > > m_SlideParticle; // 0x1790	
	// MPropertyStartGroup "Sounds"
	CSoundEventName m_strStartSound; // 0x1870	
	CSoundEventName m_strLoopingSound; // 0x1880	
	CSoundEventName m_strStopSound; // 0x1890	
};
